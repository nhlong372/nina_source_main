...
class Functions
{
    ...
    public function GetProducts($arr = array(), $class = '', $isLazy = true, $thumbsp = '')
    {
        ...
        foreach ($arr as $k => $v) {
            ...
            if (OPENDESC == true) $strpro .= '<p class="desc-product catchuoi3">' . $v['desc' . $lang] . '</p>';
                ...
                <div class="album">
                    ...
                    <?php foreach ($hinhanhsp as $k2 => $v2) { ?>
                        ...
                        <a class=" text-decoration-none none" data-fancybox="gallery<?= $v['id'] ?>" href="<?= UPLOAD_PRODUCT_L . $v2['photo'] ?>" data-caption="<?= $v['name' . $lang] ?>">
                            <p class="pic-album hover_sang2"><img onerror="this.src='<?= $thumb ?>assets/images/noimage.png';" src="<?= $thumb ?><?= UPLOAD_PRODUCT_L . $v2['photo'] ?>" alt="<?= $v2['name' . $lang] ?>" width="350" height="350" /></p>
                        </a>
                    <?php } ?>
                </div>
            <?php  }
